namespace Mapbox.Utils
{
    ...
    public struct Vector2d
    {
		...
		public static double Distance (Vector2d playerCoordinates, Vector2d objectCoordinates)
		{
			var R = 6378.137;
			var dLat = objectCoordinates.x * Mathf.PI / 180 - playerCoordinates.x * Mathf.PI / 180;
			var dLon  = objectCoordinates.y * Mathf.PI / 180 - playerCoordinates.y * Mathf.PI / 180;

			float a = Mathf.Pow (Mathf.Sin ((float)dLat / 2), 2) +
			Mathf.Cos ((float)objectCoordinates.x * Mathf.PI / 180) * Mathf.Cos ((float)playerCoordinates.x * Mathf.PI / 180) *
			Mathf.Pow (Mathf.Sin ((float)(dLon / 2)), 2);
			
			var c = 2 * Mathf.Atan2 (Mathf.Sqrt (a), Mathf.Sqrt (1 - a));
			return  R * c * 1000f; //in meters
        }
